...
class Route_Gen:
    ...
    def rotation(self, r, p, d_r, r_s, f_t):
        # r:半径　p:now position{x,y}(回転の中心　＝　{0,0}) d_r:回転の方向('left','right') r_s:rad/s f_t:一秒間に何回呼び出すか
        ...
        tan0 = p[1] / p[0]
        tan1 = math.tan(r_s / f_t)
        tan = (tan0 + tan1) / (1 - tan0 * tan1)
        d0 = sqrt(td[0] ** 2 + td[1] ** 2)
        td[0] /= d0
        td[1] /= d0
        return f_t*(tan * p[0] - p[1]) / (td[1] - tan * td[0]), td# speed,target direction{x,y}
